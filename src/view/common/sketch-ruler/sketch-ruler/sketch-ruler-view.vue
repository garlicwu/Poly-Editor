<template>
  <div class="sketch-ruler">
    ...
    <div class="canvasedit-parent" :style="rectStyle" :class="cursorClass">
      <div :id="props.canvasId" class="canvasedit" :class="cursorClass">
        <slot></slot>
      </div>
    </div>
    <!-- 水平方向 -->
    <RulerWrapper
      ...
      :parent-top="parentTop"
      @change-line-state="changeLineState"
      @apply-line-in-all-page="emit('applyLineInAllPage', $event)"
...
import Panzoom, {type PanzoomObject, type PanzoomEventDetail} from 'simple-panzoom'
import RulerLine from '@/view/common/sketch-ruler/sketch-ruler/ruler-line.vue'
...
const emit = defineEmits(['onCornerClick', 'update:scale', 'zoomchange', 'update:lockLine', 'deleteLine', 'applyLineInAllPage'])
...
const handleWheel = (e: WheelEvent) => {
  if (e.ctrlKey || e.metaKey) {
    ...
    // console.log('handleWheel', e.clientX, e.clientY)
    // console.log('ele', props.canvasId)
    // console.log('target', target)
    // if (target.id === 'drag-' + props.canvasId) {
...
const handleSpaceKeyDown = (e: KeyboardEvent) => {
  ...
  if (activeElement?.closest('.monaco-editor') || activeElement?.tagName === 'INPUT' || activeElement?.tagName === 'TEXTAREA' || activeElement?.getAttribute('contenteditable') === 'true') {
    return
  }
...
const handlePanzoomChange = (e: any) => {
  ...
  if (dimsOut) {
    ...
    startX.value = left
    elemInfo.value = e.detail?.dimsOut?.elem
    // console.log('handlePanzoomChange', elemInfo.value)
    emit('zoomchange', e.detail)
    startY.value = top
  }
}
const initPanzoom = () => {
  elem.value = document.getElementById(props.canvasId)
  if (elem.value) {
...
onUnmounted(() => {
  ...
  elem.value?.removeEventListener('panzoomchange', handlePanzoomChange)
})
...
const calculateTransform = () => {
  ...
  if (scale < 1) {
    zoomStartY = ((props.canvasHeight * scale) / 2 - props.canvasHeight / 2) / scale - (props.canvasHeight * scale - rectHeight.value) / scale / 2
  } else if (scale > 1) {
    zoomStartY = (props.canvasHeight * scale - props.canvasHeight) / 2 / scale + (rectHeight.value - props.canvasHeight * scale) / scale / 2
  } else {
    zoomStartY = 0
  }
 ...
 * @desc: 居中xy算法
 */
const calculateTransformXY = (scale: number) => {
  const zoomStartX = rectWidth.value / 2 - props.canvasWidth / 2
  let zoomStartY = 0
  if (scale < 1) {
    zoomStartY = ((props.canvasHeight * scale) / 2 - props.canvasHeight / 2) / scale - (props.canvasHeight * scale - rectHeight.value) / scale / 2
  } else if (scale > 1) {
  ...
  return {x: zoomStartX, y: zoomStartY}
}
...
watch([() => props.canvasWidth, () => props.canvasHeight, () => props.width, () => props.height], () => {
  initPanzoom()
})
watch(
  () => props.panzoomOption,
