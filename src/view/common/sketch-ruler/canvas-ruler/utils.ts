...
export function pxToMillimetre(value: number) {
  const millimetreRate = Number((defaultDpi / 25.4).toFixed(5))
  return Math.floor(value / millimetreRate).toFixed(0)
}

export function setLast(x: number, value: number, width: number, height: number, ctx: CanvasRenderingContext2D, isHorizontal?: boolean) {
  ...
  const px2Mill = pxToMillimetre(value)
  ctx.fillText(Math.round(px2Mill).toString(), 4, 7)
  ctx.restore()
  isHorizontal ? ctx.lineTo(x, height) : ctx.lineTo(width, x)
  ctx.stroke()
  ctx.closePath()
  ctx.setTransform(1, 0, 0, 1, 0, 0)
  console.log('drawEnd', isHorizontal, value, px2Mill)
}

export function drawShadowText(x: number, y: number, num: number, ctx: CanvasRenderingContext2D, palette: any, isHorizontal?: boolean) {
...
export const drawCanvasRuler = (
  ...
  if (selectLength) {
    ...
    isHorizontal ? ctx.fillRect(shadowX, 0, shadowWidth, height) : ctx.fillRect(0, shadowX, width, shadowWidth)
    ...
    if (showShadowText) {
      if (isHorizontal) {
        ...
        drawShadowText(shadowEnd, height * 0.4, Math.round(selectStart + selectLength), ctx, palette, isHorizontal)
      } else {
        drawShadowText(width * 0.4, shadowX, Math.round(selectStart), ctx, palette, isHorizontal)
        const shadowEnd = (selectStart + selectLength - start) * scale
        drawShadowText(width * 0.4, shadowEnd, Math.round(selectStart + selectLength), ctx, palette, isHorizontal)
      }
    }
  }
  ...
  for (let value = startValue10, count = 0; value < endValue; value += gridSize10, count++) {
    ...
    if (value >= 0 && value <= endNum) {
      ...
      if (endNum - value > gridSize10 / 2) {
        if (!showShadowText || selectLength == 0 || (Math.abs(value - selectStart) > gridSize10 / 2 && Math.abs(value - (selectStart + selectLength)) > gridSize10 / 2)) {
          const px2Mill = pxToMillimetre(value)
          ctx.fillText(px2Mill.toString(), 4, 9)
          console.log('draw', isHorizontal, value, px2Mill)
        }
      }
      ctx.restore()
    }
