...
import type {DragerProps, MarklineData} from '../drager'
...
export function useMarkline(targetRef: Ref<HTMLElement | null>, props: ExtractPropTypes<typeof DragerProps>) {
  ...
  const handleDrag = () => {
    ...
    // 吸附线已在 handleDragStart 中计算，无需每次 mousemove 重新计算
    const source = getBoundingClientRectByScale(targetRef.value!, props.scaleRatio)
    ...
    for (let i = 0; i < lines.value.y.length; i++) {
      ...
      // 检查下边缘
      if (Math.abs(top - sourceBottom) < props.snapThreshold) {
        markLine.diffY = top - sourceBottom
        markLine.top = showTop - parentRect.value.top
        break
      }
    }
    ...
    for (let i = 0; i < lines.value.x.length; i++) {
      ...
      // 检查右边缘
      if (Math.abs(left - sourceRight) < props.snapThreshold) {
        markLine.diffX = left - sourceRight
        markLine.left = showLeft - parentRect.value.left
        break
      }
    }
