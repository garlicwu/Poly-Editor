...
const categories = ref([{name: '', icon: '', value: ''}])
const selectedValue = ref('0')
onMounted(() => {
  categories.value = fontAfterLineHeightList.map((item) => {
    return {
      name: item,
      icon: '',
      value: item,
...
<template>
  <div class="after-line-height-container">
    <div class="after-line-height-header">
      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        <line x1="4" x2="20" y1="6" y2="6"/>
        <line x1="4" x2="20" y1="12" y2="12"/>
        <line x1="4" x2="20" y1="18" y2="18"/>
      </svg>
      <span class="header-title">段后距</span>
    </div>

    <div class="after-line-height-list">
      <div v-for="category of categories" :key="category.name" class="after-line-height-item" :class="selectedValue === category.value ? 'selected' : ''" @click="onSelectedValue(category.value)">
...
.after-line-height-list {
  @apply max-h-64 overflow-y-auto;
}

.after-line-height-item {
