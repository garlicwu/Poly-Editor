...
// 计算辅助线
export function calcLines(list: IComponentInfo[], current: IComponentInfo) {
  const lines: any = {x: [], y: []}
 ...
 * @returns 组合后的列表
 */
export function makeGroup(elements: IComponentInfo[], editorRect: DOMRect) {
  ...
  selectedItems.forEach((item) => {
    ...
    const itemRect = document.getElementById('drager-editor-main-child' + item.componentId)!.getBoundingClientRect()
    // 最小left
  ...
  // 组合组件信息
  const groupElement: IComponentInfo = {
    id: useId(),
    component: 'es-group',
    componentType: EComponentType.Group,
    group: true,
    selected: true,
    ...dragData,
 ...
 * @returns 拆分后的列表
 */
export function cancelGroup(elements: IComponentInfo[], editorRect: DOMRect) {
  // 得到当前选中元素
  const current = elements.find((item) => item.selected) as Required<IComponentInfo>
  ...
  // 获取组合元素的子元素列表
  const items = current.props.elements as IComponentInfo[]
  const newElements = items.map((item) => {
    // 子组件相对于浏览器视口位置大小
    const componentRect = document.getElementById(item.componentId!.toString())!.getBoundingClientRect()
    // 获取元素的中心点坐标
    const center = {
